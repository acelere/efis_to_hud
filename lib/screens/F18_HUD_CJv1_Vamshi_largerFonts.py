...
class F18_HUD_CJv1_Vamshi_largerFonts(Screen):
    ...
    def initDisplay(self, pygamescreen, width, height):
        ...
        self.ahrs_bg_center = (self.ahrs_bg_width / 2 + 195, self.ahrs_bg_height / 2)#Change 220 to 210, if the rotation is off center and ask again...
        ...
        self.roll_point = pygame.image.load("lib/screens/_images/Roll_Tick_Black.png").convert()
        self.roll_point.set_colorkey((0, 0, 0))
        #Roll Tick
        self.tick_GSC = pygame.image.load("lib/screens/_images/Roll_Tick_Black.png").convert()
        ...
        #Alpha
        self.alpha_symb = pygame.image.load("lib/screens/_images/Alpha_Letter_LC.png").convert()
        self.alpha_symb.set_colorkey((0, 0, 0))
        self.alpha_symb = pygame.transform.scale(self.alpha_symb, (15, 15))
        ...
        self.fontsmallest = pygame.font.SysFont("monospace", 16, bold=False)  # units
        #Special Characters font
        self.specialFont = pygame.font.SysFont("Source Code Pro", 22, bold=False)  # units
...
#             pygame.draw.line(self.roll_tick, (255, 0, 255), [x0, y0], [x1, y1], 2)
    ...
    def draw(self, aircraft, FPS):
        ...
        if self.alt_box_mode:
            ...
            #label = self.myfont.render("%sz" % datetime.datetime.fromtimestamp(float(aircraft.sys_time_string.split()[0])), 1, (255, 255, 0))
            ...
            if aircraft.vsi < 0:
                ...
                label = self.fontIndicatorSmaller.render("+%d" % aircraft.vsi, 1, (2, 255, 20))
            self.pygamescreen.blit(label, (self.width - 90, self.heightCenter - 50))
            # True aispeed
            ...
            #Ground Speed Cuing
#             label = self.myfont.render("|", 1, (2, 255, 20))
...
#            label_time = self.myfont.render(
#                "%s" % time_now, 1, (2, 255, 20)
#            )
#            self.pygamescreen.blit(label_time, (self.width - 90, self.heightCenter + 220))

            time_now_a_c = datetime.datetime.fromtimestamp(float(aircraft.sys_time_string)).strftime("%H:%M:%S")
            label = self.myfont.render("%s" % time_now_a_c, 1, (2, 255, 20))
            self.pygamescreen.blit(label, (self.width - 90, self.heightCenter + 220))
            
#             label = self.myfont.render("GS %d" % aircraft.gndspeed, 1, (2, 255, 20))
#             self.pygamescreen.blit(label, (05, self.heightCenter + 50))
            # Ground speed
            label = self.myfont.render("GS %d" % aircraft.gndspeed, 1, (2, 255, 20))
            self.pygamescreen.blit(label, (05, self.heightCenter + 50))
            # Vertical G
            label = self.myfont.render("G %0.1f" % aircraft.vert_G, 1, (2, 255, 20))
            self.pygamescreen.blit(label, (25, self.heightCenter + 110))
            # Max G
            label = self.myfont.render("%0.1f" % max_G_Static, 1, (2, 255, 20))
            self.pygamescreen.blit(label, (45, self.heightCenter + 130))
            #Mach Number
            label = self.myfont.render("M %0.1f" % aircraft.vert_G, 1, (2, 255, 20))
            self.pygamescreen.blit(label, (25, self.heightCenter + 90))
            #Alpha Number            
            self.pygamescreen.blit(
                self.alpha_symb, (30, self.heightCenter + 72)
            )
            label = self.myfont.render("%0.1f" % aircraft.vert_G, 1, (2, 255, 20))
            self.pygamescreen.blit(label, (48, self.heightCenter + 70))
            
#            if aircraft.pitch >= 0 and aircraft.pitch <= 10:
#                #Nadir Circle            
#                self.pygamescreen.blit(
#                    self.Nadir_Circ, (215, self.heightCenter - 20)
#                    )
#            elif aircraft.pitch >= -10 and aircraft.pitch <= -1:
#                #Zenith Star            
#                self.pygamescreen.blit(
#                   self.Zenith_Star, (215, self.heightCenter - 20)
#                    )
                
            #Wind Dir/Speed
            label = self.myfont.render("%d/" % aircraft.wind_dir, 1, (2, 255, 20))#, % aircraft.wind_speed
            self.pygamescreen.blit(label, (15, self.heightCenter + 220))
            
            label = self.myfont.render("%d" % aircraft.wind_speed, 1, (2, 255, 20))#, % aircraft.wind_speed
...
#         
        ...
        # main HDG processing
        _hdg.hdg_main(self, aircraft.mag_head, aircraft.gndtrack, aircraft.turn_rate)#aircraft.mag_head
